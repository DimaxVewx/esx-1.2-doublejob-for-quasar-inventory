...
ESX.SavePlayer = function(xPlayer, cb)
    ...
    table.insert(asyncTasks, function(cb2)
        MySQL.Async.execute('UPDATE users SET accounts = @accounts, job = @job, job2 = @job2, job_grade2 = @job_grade2, job_grade = @job_grade, `group` = @group, loadout = @loadout, position = @position WHERE identifier = @identifier', {
            ['@accounts'] = json.encode(xPlayer.getAccounts(true)),
            ['@job'] = xPlayer.job.name,
    ...
    Async.parallel(asyncTasks, function(results)
        print(('[es_extended] [^2INFO^7] Sauvegarde du joueur "%s^7"'):format(xPlayer.getName()))
...
ESX.SavePlayers = function(cb)
    ...
    Async.parallelLimit(asyncTasks, 8, function(results)
        print(('[es_extended] [^2INFO^7] Sauvegarde %s des joueurs'):format(#xPlayers))
        if cb then
            cb()
        end
    end)
