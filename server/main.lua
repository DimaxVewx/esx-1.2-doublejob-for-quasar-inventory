...
function onPlayerJoined(playerId)
    local identifier
    local license
	local joueursj = GetPlayerName(playerId)
        ...
        if ESX.GetPlayerFromIdentifier(identifier) then
            ...
            MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier', {
                ...
                if result then
                    ...
                    MySQL.Async.execute('INSERT INTO users (accounts, identifier, license, name) VALUES (@accounts, @identifier, @license, @name)', {
...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job2, job_grade, job_grade2, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
			local job, grade, jobObject, gradeObject = result[1].job, tostring(result[1].job_grade)
			local job2, grade2, jobObject2, gradeObject2 = result[1].job2, tostring(result[1].job_grade2)
			...
			if gradeObject.skin_male then userData.job2.skin_male = json.decode(gradeObject.skin_male) end
			if gradeObject.skin_female then userData.job2.skin_female = json.decode(gradeObject.skin_female) end
			...
			userData.job.grade_name = gradeObject.name
			userData.job.grade_label = gradeObject.label
			userData.job.grade_salary = gradeObject2.salary
	...
	Async.parallel(tasks, function(results)
		...
		print(('[^2INFO^7] Le joueur "%s^7" vient de se connecter, désormais id %s'):format(xPlayer.getName(), playerId))
	end)
end
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local pickupLabel = ('~r~%s~s~ [~r~%s~s~]'):format(xItem.label, itemCount)
	...
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('~r~%s~s~ [~r~%s~s~]'):format(account.label, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weaponObject.ammo and weapon.ammo > 0 then
				local ammoLabel = weaponObject.ammo.label
				pickupLabel = ('~r~%s~s~ [~r~%s~s~ %s]'):format(weapon.label, weapon.ammo, ammoLabel)
				xPlayer.showNotification(_U('threw_weapon_ammo', weapon.label, weapon.ammo, ammoLabel))
			else
				pickupLabel = ('~r~%s~s~'):format(weapon.label)
